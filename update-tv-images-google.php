...
echo "🔄 Actualizare imagini TV-uri cu Google Custom Search API...\n\n";

$tvs = App\Models\Product::where('product_type_id', 8)
    ->where('image_url', 'LIKE', '%ui-avatars.com%')
    ->get();

echo "📺 Găsite {$tvs->count()} TV-uri fără imagini reale\n\n";
...
foreach ($tvs as $tv) {
    ...
    $imageUrl = fetchImageFromGoogle($tv->name, $apiKey, $searchEngineId);
    
    if ($imageUrl) {
        $tv->image_url = $imageUrl;
        $tv->save();
        $updated++;
        echo "  ✅ Imagine actualizată\n";
    } else {
        $failed++;
...
echo "  - Eșuate: $failed\n";
echo "  - Request-uri Google folosite: " . ($updated + $failed) . "/100\n";
